...
public class Process {
    ...
    static double avgTurnaroundTime(Process oTasksList[]){
        int turnaroundTimeSum = 0, size = (oTasksList.length == 0) ? 1 : oTasksList.length;
        for(int i=0; i<size; i++){
    ...
    static void printProcessArray(Process oList[]){
        ...
        for (int i=0; i<oList.length; i++){
            fmt.format("%3s %3s %5s %8s %12s %13s %18s %6s %8s %12s %14s %16s\n", i+1, oList[i].getId(), oList[i].getState(), oList[i].getPriority(), oList[i].getArrivalTime(), oList[i].getResponseTime(), oList[i].getBurst_time()+" Start("+oList[i].getSBurstTime()+")", oList[i].getIO(), oList[i].getIoStartTime(), oList[i].getIoWaitTime(), oList[i].getTimeFinished(), oList[i].getRemain_TimeSlice());
        }
