...
public class Process {
    ...
    static void printProcessArray(Process oList[]){
        ...
        fmt.format("%4s %3s %7s %9s %11s %12s %12s %11s %6s %10s %10s %10s %12s %7s\n", "#|", "ID |", "State |", "Priority |", "ArivalTime |", "ResponseTime |", "TurnaroundTime |", "Burst_Time |", "IO |", "ioStartTime |", "ioWaitTime |", "TimeStarted |", "TimeFinished |", "Remain_TimeSlice |"); 
        
        for (int i=0; i<oList.length; i++){
            fmt.format("%3s %3s %5s %8s %12s %13s %15s %19s %6s %8s %12s %13s %13s %16s\n", i+1, oList[i].getId(), oList[i].getState(), oList[i].getPriority(), oList[i].getArrivalTime(), oList[i].getResponseTime(), oList[i].getTurnaroundTime(), oList[i].getBurst_time()+" Start("+oList[i].getSBurstTime()+")", oList[i].getIO(), oList[i].getIoStartTime(), oList[i].getIoWaitTime(), oList[i].timeStarted, oList[i].getTimeFinished(), oList[i].getRemain_TimeSlice());
        }
